...
if [ "$CLUSTER" = "DeltaManager" -o "$CLUSTER" = "BackupManager" ]; then
  if [ -e "server-cluster.xsl" ]; then
  	SERVER_PARAMS="$SERVER_PARAMS --param CLUSTER \"$CLUSTER\" --param REPLICATION_FILTER \"$REPLICATION_FILTER\" --param CHANNEL_SEND_OPTIONS \"$CHANNEL_SEND_OPTIONS\""
	...
	if [ -n "$MCAST_ADDRESS" ]; then
		...
		SERVER_PARAMS="$SERVER_PARAMS --param RECEIVE_PORT \"${RECEIVE_PORT:-5001}\" --param KUBERNETES_NAMESPACE \"$KUBERNETES_NAMESPACE\" --param OPENSHIFT_KUBE_PING_NAMESPACE \"$OPENSHIFT_KUBE_PING_NAMESPACE\" --param DNS_MEMBERSHIP_SERVICE_NAME \"$DNS_MEMBERSHIP_SERVICE_NAME\""
